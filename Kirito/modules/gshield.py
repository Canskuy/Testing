...
from KiritoRobot.conf import get_int_key, get_str_key
from KiritoRobot.pyrogramee.telethonbasics import is_admin
from KiritoRobot.events import register
from KiritoRobot.modules.sql_extended.nsfw_watch_sql import (
    add_nsfwatch,
    get_all_nsfw_enabled_chat,
    is_nsfwatch_indb,
    rmnsfwatch,
)
from KiritoRobot import telethn as tbot, BOT_ID
from KiritoRobot.mongo import db
        ...
        if event.photo:
            if event.chat_id == c["id"]:
                ...
                if nude.is_nude("./nudes.jpg"):
                    ...
                    st = sender.first_name
                    hh = sender.id
                    final = f"**NSFW DETECTED**\n\n{st}](tg://user?id={hh}) your message contain NSFW content.. So, Kirito deleted the message\n\n **Nsfw Sender - User / Bot :** {st}](tg://user?id={hh})  \n\n`⚔️Automatic Detections Powered By KiritoAI` \n**#GROUP_GUARDIAN** "
                    dev = await event.respond(final)
                    await asyncio.sleep(10)
                    await dev.delete()
                    os.remove("nudes.jpg")
