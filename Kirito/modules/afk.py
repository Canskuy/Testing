...
from KiritoRobot.modules.sql import afk_sql as sql
import KiritoRobot.modules.helper_funcs.fun_strings as fun
from KiritoRobot.modules.users import get_user_id
    ...
    if res:
        ...
        try:
            options = [
                ...
                "{} ngapain pulang mending cabut aja",
                "{} online mulu, jomblo ya",
                "{} mending mati nanggung off doang",
                "Heh {}, emak lu nyariin",
                "{} percuma balik lagi ga ada yang nyari loe",
                "{} gila ya lu ?",
                "{} kenapa kamu nyebelin si ?",
                "{} ga ada yang peduli lu off",
                "{} idola kalian baru aktif 😜",
                " liat, {} anak caper balik lagi",
...
def check_afk(update, context, user_id, fst_name, userc_id):
    if sql.is_afk(user_id):
        ...
        if int(userc_id) == int(user_id):
            return
        if not user.reason:
            res = "I'm busy right now".format(fst_name)
