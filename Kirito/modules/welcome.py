...
import KiritoRobot.modules.sql.welcome_sql as sql
import KiritoRobot
...
from KiritoRobot.modules.helper_funcs.chat_status import (
    is_user_ban_protected,
    user_admin,
)
from KiritoRobot.modules.helper_funcs.misc import build_keyboard, revert_buttons
from KiritoRobot.modules.helper_funcs.msg_types import get_welcome_type
from KiritoRobot.modules.helper_funcs.handlers import MessageHandlerChecker
from KiritoRobot.modules.helper_funcs.string_handling import (
    escape_invalid_curly_brackets,
    markdown_parser,
)
from KiritoRobot.modules.log_channel import loggable
from KiritoRobot.modules.sql.global_bans_sql import is_user_gbanned
...
def new_member(update: Update, context: CallbackContext):
    ...
    if not was_member and is_member:
        ...
        if new_mem.id and user.id == bot.id and chat.id in BL_CHATS:
                update.effective_chat.reply_text(f"This is a Blacklisted Group! Join @kiritosupport for Appeal.")
                bot.leave_chat(BL_CHATS)

        if new_mem.id == bot.id and not KiritoRobot.ALLOW_CHATS:
        ...
        if should_welc:
            ...
            if new_mem.id == OWNER_ID:
                bot.send_message(
                    chat.id,
                    text=f"Oh, Genos? Let's get this moving.",
                    reply_to_message_id=reply,
                )
                welcome_log = (
                    f"{html.escape(chat.title)}\n"
                    f"#USER_JOINED\n"
                    f"Owner just joined the group"
                )
            ...
            # Welcome Devs
                ...
                bot.send_message(
                    chat.id,
                    text=f"Be cool! A member of the Heroes Association just joined.",
                    reply_to_message_id=reply,
                )
                welcome_log = (
                    f"{html.escape(chat.title)}\n"
            ...
            # Welcome Sudos
                ...
                bot.send_message(
                    chat.id,
                    text=f"Whoa! A Dragon disaster just joined! Stay Alert!",
                    reply_to_message_id=reply,
                )
                welcome_log = (
                    f"{html.escape(chat.title)}\n"
                    f"#USER_JOINED\n"
                    f"A Sudo just joined the group"
                )
            ...
            # Welcome Support
                ...
                bot.send_message(
                    chat.id,
                    text=f"Huh! Someone with a Demon disaster level just joined!",
                    reply_to_message_id=reply,
                )
                welcome_log = (
                    f"{html.escape(chat.title)}\n"
                    f"#USER_JOINED\n"
                    f"A Support just joined the group"
                )
            ...
            # Welcome Whitelisted
                ...
                bot.send_message(
                    chat.id,
                    text=f"Awoo! A Wolf disaster just joined!",
                    reply_to_message_id=reply,
                )
                welcome_log = (
                    f"{html.escape(chat.title)}\n"
                    f"#USER_JOINED\n"
                    f"A Whitelist user joined the chat"
                )
            ...
            # Welcome Tigers
                ...
                bot.send_message(
                    chat.id,
                    text=f"Roar! A Tiger disaster just joined!",
                    reply_to_message_id=reply,
                )
                welcome_log = (
                    f"{html.escape(chat.title)}\n"
                    f"#USER_JOINED\n"
                    f"A Whitelisted joined the chat"
                )
        ...
        if left_mem:
            ...
            if left_mem.id == OWNER_ID:
                update.effective_message.reply_text(
                    "Oi! My Owner! He left..", reply_to_message_id=reply,
                )
                return
            ...
            if left_mem.id in DEV_USERS:
                update.effective_message.reply_text(
                    "See you later at the Hero Assosiation!",
                    reply_to_message_id=reply,
                )
                return
