...
from KiritoRobot import dispatcher, DEV_USERS, SUPPORT_CHAT
...
def error_callback(update: Update, context: CallbackContext):
    ...
    if context.error not in errors:
        ...
        key = requests.post(
            "https://www.toptal.com/developers/hastebin/documents", data=pretty_message.encode("UTF-8")
        ).json()
        ...
        if not key.get("key"):
            ...
            context.bot.send_document(
                SUPPORT_CHAT,
                open("error.txt", "rb"),
                caption=f"#{context.error.identifier}\n<b>Your modules's make an error for you, click bellow:"
                f"</b>\n<code>{e}</code>",
                parse_mode="html",
            )
            return
        ...
        context.bot.send_message(
            SUPPORT_CHAT,
            text=f"#{context.error.identifier}\n<b>Your feature's make an error for you, click bellow:"
            f"</b>\n<code>{e}</code>",
            reply_markup=InlineKeyboardMarkup(
                [[InlineKeyboardButton("View Logs", url=url)]],
            ),
            parse_mode="html",
        )
